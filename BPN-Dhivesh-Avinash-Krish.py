...
le = LabelEncoder()
targets = pd.DataFrame(data=le.fit_transform(df['typeoffault_n']*df['lineno']))
...
#NOTE: ENCODE CLASS VALUES AS INTEGERS encode class values as integers
encoder = LabelEncoder()
encoder.fit(dataset2[:])
encoded_Y = encoder.transform(dataset2[:])
#NOTE: CREATING THE BACKPROPAGATION MODEL 
def baseline_model():
    ...
    return model
#NOTE: CONVERTING INTEGERS TO DUMMY VARIABLES 
dummy_y = np_utils.to_categorical(encoded_Y)
#NOTE: TRAINING DATASET
estimator = KerasClassifier(build_fn=baseline_model, epochs=10, batch_size=5, verbose=1) 
kfold = KFold(n_splits=3, shuffle=True)
results = cross_val_score(estimator, dataset1[:,:], dummy_y, cv=kfold) #NOTE: TESTING DATASET
...
r = pd.DataFrame(estimator.predict(X_test[:5000]))
for i in r[0].head():
    print(output[str(i)])
